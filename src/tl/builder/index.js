const builderMap = {
  ...
  'messageActionGiftCode': function(params) {
    ...
    const flags = (this.has(params.via_giveaway) << 0) | (this.has(params.unclaimed) << 2) | (this.has(params.boost_peer) << 1) | (this.has(params.currency) << 2) | (this.has(params.amount) << 2) | (this.has(params.crypto_currency) << 3) | (this.has(params.crypto_amount) << 3) | (this.has(params.message) << 4);
    this.int32(flags);
    this.flag(this.predicate, params.boost_peer);
    this.int(params.months);
    this.string(params.slug);
